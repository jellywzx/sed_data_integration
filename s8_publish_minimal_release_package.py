...
- Minimal satellite NetCDF files are `sed_reference_satellite_daily.nc` and
  `sed_reference_satellite_monthly.nc`; the full `sed_reference_satellite.nc`
  remains only in the full release package.
...
def copy_integrated_extension_files(args):
    ...
    for name in files:
        ...
        if src.is_file():
            ...
            elif name in SATELLITE_NC_FILES:
                if args.dry_run:
...
def validate_minimal_package(args):
    ...
    if not args.skip_satellite:
        for satellite_name in SATELLITE_NC_FILES:
            ...
            expected_resolution = "daily" if satellite_name == SATELLITE_DAILY_NC else "monthly"
            try:
