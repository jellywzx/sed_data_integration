...
SATELLITE_FULL_NC = "sed_reference_satellite.nc"
SATELLITE_DAILY_NC = "sed_reference_satellite_daily.nc"
SATELLITE_MONTHLY_NC = "sed_reference_satellite_monthly.nc"
SATELLITE_CATALOG_CSV = "satellite_catalog.csv"
SATELLITE_SUBSET_NC_FILES = (
    SATELLITE_DAILY_NC,
    SATELLITE_MONTHLY_NC,
)
SATELLITE_FULL_NC_FILES = (SATELLITE_FULL_NC,)
SATELLITE_ALL_NC_FILES = SATELLITE_FULL_NC_FILES + SATELLITE_SUBSET_NC_FILES
# Union of every possible satellite file; used only for inventory status
# classification (membership test), not for selecting which files to build.
SATELLITE_PACKAGE_FILES = SATELLITE_ALL_NC_FILES + (SATELLITE_CATALOG_CSV,)
INTEGRATED_EXTENSION_FILES = CLIMATOLOGY_PACKAGE_FILES + SATELLITE_PACKAGE_FILES


def satellite_nc_files(args):
    """Active satellite NetCDF filenames for the minimal package."""
    if getattr(args, "satellite_daily_monthly", False):
        return SATELLITE_SUBSET_NC_FILES
    return SATELLITE_FULL_NC_FILES


def satellite_package_files(args):
    """Active satellite files (NetCDF + catalog) for the minimal package."""
    return satellite_nc_files(args) + (SATELLITE_CATALOG_CSV,)
...
COMMON_GLOBAL_ATTRS = {
    ...
    "id": "https://doi.org/10.5281/zenodo.21896563",
...
DAILY_MATRIX_GLOBAL_ATTRS = {
    **COMMON_GLOBAL_ATTRS,
    ...
    "product_id": "{}.station-reference.daily".format(BASE_ID),
...
MONTHLY_MATRIX_GLOBAL_ATTRS = {
    **COMMON_GLOBAL_ATTRS,
    ...
    "product_id": "{}.station-reference.monthly".format(BASE_ID),
    "source": _MATRIX_SOURCE,
    "history": (
...
ANNUAL_MATRIX_GLOBAL_ATTRS = {
    **COMMON_GLOBAL_ATTRS,
    ...
    "product_id": "{}.station-reference.annual".format(BASE_ID),
    "source": _MATRIX_SOURCE,
    "history": (
...
CLIMATOLOGY_GLOBAL_ATTRS_DEFAULTS = {
    **COMMON_GLOBAL_ATTRS,
    ...
    "product_id": "{}.climatology".format(BASE_ID),
    "source": (
...
SATELLITE_GLOBAL_ATTRS_DEFAULTS = {
    **COMMON_GLOBAL_ATTRS,
    ...
    "history": (
        ...
        "linked to eligible stations in the main station-reference component by the "
        "sed_data_integration harmonization and publication pipeline."
    ).format(CURRENT_UTC),
}
...
GLOBAL_ATTRS_BY_FILE = {
    ...
    SATELLITE_DAILY_NC: dict(SATELLITE_GLOBAL_ATTRS_DEFAULTS, time_coverage_resolution="daily"),
    SATELLITE_MONTHLY_NC: dict(SATELLITE_GLOBAL_ATTRS_DEFAULTS, time_coverage_resolution="monthly"),
}
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--satellite-daily-monthly",
        action="store_true",
        help=(
            "Replace the full sed_reference_satellite.nc in the minimal package with "
            "sed_reference_satellite_daily.nc and sed_reference_satellite_monthly.nc subsets."
        ),
    )
    parser.add_argument(
        "--compression-level",
...
def _minimal_satellite_global_attrs(src, variable_names, src_path):
    ...
    attrs["time_coverage_resolution"] = (
        _clean_attr_value(_source_attr(src, "time_coverage_resolution", ""))
        or attrs.get("time_coverage_resolution", "")
    )
...
_MINIMAL_ALIASES_MS = {
    ...
    "GSED": ["GSED"],
    "Dethier": ["Dethier"],
    "RivSed": ["RiverSed", "RiverSed (USA)", "RivSed"],
}
...
- The minimal satellite NetCDF is the full `sed_reference_satellite.nc` by
  default; building with `--satellite-daily-monthly` instead ships
  `sed_reference_satellite_daily.nc` and `sed_reference_satellite_monthly.nc`.
- `satellite_catalog.csv` remains the station-level satellite summary used by
  the workflow and is retained for compatibility.
- S9 public packaging generates `satellite_query_catalog.csv.gz` directly from
  the final public `sed_reference_satellite.nc`; this is the record-level query
  catalogue corresponding to the satellite NetCDF fields documented in SP Table S11.
...
- Join source provenance with `selected_source_station_uid` ->
  ...
  `source_dataset_catalog.source_name`; the station-level satellite summary joins
  through `source_name`, while the satellite query catalogue and climatology
  catalogue expose the corresponding public `source` field.
...
def integrated_extension_files(args):
    ...
    if args.skip_satellite:
        skipped.extend(satellite_package_files(args))
    else:
        files.extend(satellite_package_files(args))
...
def copy_integrated_extension_files(args):
    ...
    for name in files:
        ...
        if src.is_file():
            ...
            elif name in satellite_nc_files(args):
                if args.dry_run:
...
def validate_minimal_package(args):
    ...
    if not args.skip_satellite:
        for satellite_name in satellite_nc_files(args):
            satellite_nc_path = args.minimal_dir / satellite_name
            if not satellite_nc_path.is_file():
                add(
                    "satellite_variables:{}".format(satellite_name),
                    "fail",
            ...
            if satellite_variables:
                ...
                add(
                    "satellite_required_vars:{}".format(satellite_name),
                ...
                add(
                    "satellite_forbidden_vars:{}".format(satellite_name),
            ...
            try:
                ...
                add(
                    "satellite_global_attrs:{}".format(satellite_name),
                    "fail",
            ...
            if satellite_name == SATELLITE_DAILY_NC:
                expected_resolution = "daily"
            elif satellite_name == SATELLITE_MONTHLY_NC:
                expected_resolution = "monthly"
            else:
                expected_resolution = None  # full satellite file: no single resolution
            if expected_resolution is not None:
                try:
                    with nc4.Dataset(satellite_nc_path, "r") as ds:
                        actual_resolution = _clean_ms(getattr(ds, "time_coverage_resolution", ""))
                except Exception as exc:
                    add(
                        "satellite_time_coverage_resolution:{}".format(satellite_name),
                        "fail",
                        "cannot inspect satellite time_coverage_resolution",
                        str(exc),
                    )
                else:
                    add(
                        "satellite_time_coverage_resolution:{}".format(satellite_name),
                        "pass" if actual_resolution == expected_resolution else "fail",
                        "satellite time_coverage_resolution matches subset"
                        if actual_resolution == expected_resolution
                        else "satellite time_coverage_resolution differs from subset",
                        "expected={}; actual={}".format(expected_resolution, actual_resolution or "(empty)"),
                    )
    ...
    active_satellite_ncs = set(satellite_nc_files(args))
    for name in SATELLITE_ALL_NC_FILES:
        if name in active_satellite_ncs:
            continue
        path = args.minimal_dir / name
        add(
            "forbidden_file:{}".format(name),
            "fail" if path.exists() else "pass",
            "inactive satellite NetCDF absent"
            if not path.exists()
            else "inactive satellite NetCDF present",
            str(path),
        )
    ...
    if not args.skip_satellite:
        for satellite_name in satellite_nc_files(args):
            parity_products.append(
                (
                    satellite_name,
                    args.release_dir / satellite_name,
                    args.minimal_dir / satellite_name,
                )
            )
