...
Usage:
  ...
  python validate/diagnose_riversed_dethier_spatial_overlap_maps.py \
  python validate/diagnose_dethier_spatial_overlap_maps.py \
...
OUTPUT_DIR = _SCRIPT_DIR / "output" / "dethier"
...
# ── Dethier regional bounding boxes (global continents) ─────────────────────
DETHIER_REGIONS = {
    "Global": (-180.0, -60.0, 180.0, 80.0),
    "NorthAmerica": (-140.0, 15.0, -50.0, 75.0),
    "SouthAmerica": (-85.0, -60.0, -30.0, 15.0),
    "Europe": (-10.0, 35.0, 40.0, 72.0),
    "Africa": (-20.0, -35.0, 55.0, 38.0),
    "AsiaOceania": (25.0, -50.0, 180.0, 75.0),
}
...
def assign_dethier_continent(lat, lon):
    """Assign continent tag for Dethier stations."""
    # North America
    if -140 <= lon <= -50 and 15 <= lat <= 75:
        return "NorthAmerica"
    # South America
...
def build_diagnostic_table(source_df, source_name, cluster_summary_df):
    ...
    for _, row in source_df.iterrows():
        ...
        region = assign_dethier_continent(lat, lon)
...
def generate_global_overview(source_df, source_name, main_pts, out_dir,
                             global_extent=None):
    ...
    if global_extent is None:
        global_extent = (-180, -60, 180, 80)
    title_base = "Dethier — Main Cluster Spatial Overview (Global)"
    figsize = (16, 8)
    ...
    fname = "global_overview_dethier.png"
...
def generate_regional_zoom(region_name, bbox, source_df,
    ...
    print("\n  Generating dethier_{}_zoom.png ...".format(region_name))

    # Dethier: MERIT reaches are CONUS-only, not applicable
    ...
    if source_df is not None and len(source_df) > 0:
        ...
        if len(unlinked) > 0:
            ax.scatter(
                ...
                **({} if not use_cartopy else {"transform": ccrs.PlateCarree()}),
                label="Dethier unlinked", zorder=6,
            )
    ...
    ax.set_title("{} — Dethier spatial overlap".format(region_name),
                 fontsize=12)

    if use_cartopy and bbox[2] - bbox[0] < 50:
        _add_scale_bar(ax, bbox, length_km=100)

    fname = "dethier_{}_zoom.png".format(region_name.lower())
    out_path = out_dir / fname
    fig.savefig(str(out_path), dpi=200, bbox_inches="tight")
    plt.close(fig)
    print("    -> {}".format(out_path))
...
def generate_basin_diagnostics(source_df, main_pts,
    ...
    print("\n  Generating basin diagnostic maps for Dethier ...")
    ...
    for cat_name, cat_df_cat, color in categories:
        ...
        for idx_i, (_, row) in enumerate(examples.iterrows()):
            ...
            # MERIT reaches: CONUS-only, not applicable for Dethier
            ...
            ax.scatter(
                ...
                **({} if not use_cartopy else {"transform": ccrs.PlateCarree()}),
                label="Dethier ({})".format(cat_name), zorder=6,
            )
            ...
            fname = "basin_diag_dethier_{}_{}.png".format(cat_name, idx_i)
            out_path = out_dir / fname
            fig.savefig(str(out_path), dpi=200, bbox_inches="tight")
            plt.close(fig)
            print("    -> {}".format(out_path))
...
def process_source(source_name, linkage_df, cluster_summary_df,
    ...
    # ── 5a. Global overview ────────────────────────────────────────────────
    generate_global_overview(src_df, source_name, cluster_summary_df, out_dir)

    # ── 5b. Regional zooms ─────────────────────────────────────────────────
    for region_name, region_bbox in DETHIER_REGIONS.items():
