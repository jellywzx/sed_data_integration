...
DEFAULT_OUT_DIR = REPO_ROOT / "validate" / "output" / "basin_matching_sensitivity"
...
def _process_station_chunk(station_rows: List[dict]) -> List[dict]:
...
def load_station_scope(
    ...
    if scope == "release_main":
        if s5_csv is None or not s5_csv.is_file():
            ...
            s5 = pd.read_csv(s5_csv, usecols=lambda name: name in {"station_key"})
            _require_columns(s5, ["station_key"], "s5 CSV")
            released_keys = set(s5["station_key"].fillna("").astype(str).str.strip())
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    chunks = list(_chunked(station_records, int(args.chunk_size)))
