...
def plot_overlay_source_contribution(
    ...
    fig.subplots_adjust(left=0.22, right=0.97, top=0.86, bottom=0.14)
...
def main(argv=None) -> int:
    ...
    if Path(__file__).resolve() != Path(script_copy_b).resolve():
    ...
    if Path(__file__).resolve() != Path(script_copy_c).resolve():
