...
def draw_satellite_map(ax, satellite: pd.DataFrame, satellite_area: pd.DataFrame) -> None:
    ...
    df["source"] = df["source"].replace("RiverSed", "RivSed")
    for source, group in df.groupby("source", dropna=False):
        color = SOURCE_COLORS.get(source, SOURCE_COLORS["Unknown"])
        marker_size = 4 if source == "RivSed" else 7
        alpha = 0.25 if source == "RivSed" else 0.55
...
def draw_climatology_timeseries_map(ax, climatology: pd.DataFrame, timeseries: dict[str, pd.DataFrame]) -> None:
    ...
    for resolution in ("daily", "monthly", "annual", "climatology", "other"):
        ...
        is_background = resolution == "climatology"
        ax.scatter(
            group["lon"],
            group["lat"],
            s=3 if is_background else 8,
            c=RESOLUTION_COLORS[resolution],
            alpha=0.8 if is_background else 0.55,
            linewidths=0.0 if is_background else 0.12,
            edgecolors="none" if is_background else ("black" if marker != "x" else RESOLUTION_COLORS[resolution]),
            marker=marker,
            rasterized=True,
            zorder=1 if is_background else 2,
        )
    ...
    climatology_count = int(climatology["resolution"].eq("climatology").sum())
    if climatology_count:
        items.append(("Climatology ({})".format(climatology_count), RESOLUTION_COLORS["climatology"], RESOLUTION_MARKERS["climatology"]))
