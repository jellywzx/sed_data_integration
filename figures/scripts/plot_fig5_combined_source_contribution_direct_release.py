...
def load_other_product_sources_from_minimal(
    ...
    empty = pd.DataFrame(
        columns=["source_name", "contribution_count", "record_count", "first_year", "last_year"]
    )

    # Climatology has always used station_uid in the release catalog.
    ...
    # Satellite catalog remains on the existing cluster_uid contract.
...
def annotate_cluster_counts_on_twin(
    ...
    del label_ax  # kept in the signature for parity with the previous helper
