...
def build_table5(spatial_by_resolution: pd.DataFrame, spatial_summary: pd.DataFrame, temporal: pd.DataFrame) -> pd.DataFrame:
    ...
    if not unique_stations:
        ...
        unique_stations = _summary_metric(spatial_summary, "cluster_count")
...
def build_report(
    ...
    lines.extend(
        [
            ...
            "",
            table6[["Resolution", "Variable", "Non-missing values, n (%)", "Median", "P05–P95", "P99"]].to_markdown(index=False),
            "",
            "## Table 7",
            "",
...
def main(argv=None) -> int:
    ...
    flag01_summary = _read_csv(
        ...
        {"resolution", "variable", "n_flag01_records", "median", "p05", "p95", "p99", "unit"},
