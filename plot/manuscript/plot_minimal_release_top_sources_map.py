...
def add_categories(points: pd.DataFrame, top_sources: List[str]) -> pd.DataFrame:
    ...
    out["category"] = np.where(out["source_name"].isin(top_set), out["source_name"], OTHER_LABEL)
    return out
...
def plot_map(points: pd.DataFrame, top_sources: List[str], counts: Dict[str, int],
    ...
    color_map = {source: OKABE_ITO_EXTENDED[i % len(OKABE_ITO_EXTENDED)] for i, source in enumerate(top_sources)}
    ...
    other = points[points["category"].eq(OTHER_LABEL)]
    if not other.empty:
        scatter_points(ax, other, "#999999", OTHER_LABEL, size=8, alpha=1.0, zorder=2)

    for idx, source in enumerate(top_sources):
        subset = points[points["category"].eq(source)]
        if subset.empty:
            continue
        scatter_points(ax, subset, color_map[source], source, size=8, alpha=0.72, zorder=3 + idx, marker=TOP_MARKERS[idx % len(TOP_MARKERS)])
    ...
    legend_order = top_sources + [OTHER_LABEL]
    handles = []
    for idx, label in enumerate(legend_order):
        ...
        if label == OTHER_LABEL:
            ...
            marker = TOP_MARKERS[idx % len(TOP_MARKERS)]
        handles.append(
            Line2D(
                [0],
                [0],
...
def print_summary(catalog: pd.DataFrame, points: pd.DataFrame, input_counts: pd.Series, top_sources: List[str]) -> None:
    ...
    print("  {}: {:,}".format(OTHER_LABEL, int(category_counts.get(OTHER_LABEL, 0))))
