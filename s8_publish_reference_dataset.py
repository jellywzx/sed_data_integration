...
def main():
    ...
    # Also register satellite_validation_catalog.csv alias if it exists
    val_cat_path = out_dir / "satellite_validation_catalog.csv"
    if val_cat_path.is_file():
        file_records.append(
            (
                "satellite_catalog",
                val_cat_path,
                "Alias for satellite_catalog.csv (backward-compatible name)",
            )
        )
    ...
    if DEFAULT_SKIP_OVERLAP_CANDIDATES:
        ...
        if built_path is None:
            if overlap_candidates_path.is_file():
                print("Note: overlap candidate sidecar exists from prior run; adding to inventory.")
                overlap_candidates_validation_path = overlap_candidates_path
                file_records.append(
                    (
                        "provenance_sidecar",
                        overlap_candidates_path,
                        "Pre-existing candidate-level values for multi-source overlap validation",
                    )
                )
            else:
    ...
    # Register other existing sidecar files (generated by upstream s6 scripts)
    for p, kind, desc in [
        (out_dir / "sed_reference_overlap_candidates.parquet", "provenance_sidecar",
         "Parquet version of overlap candidates"),
        (out_dir / "sed_reference_satellite_candidates.csv.gz", "provenance_sidecar",
         "Satellite validation-only candidate records (CSV)"),
        (out_dir / "sed_reference_satellite_candidates.parquet", "provenance_sidecar",
         "Parquet version of satellite candidates"),
    ]:
        if p.is_file():
            file_records.append((kind, p, desc))
    if args.skip_gpkg:
    ...
    app_readme_path = out_dir / "application_sed_reference_release.md"
    if app_readme_path.is_file():
        file_records.append(
            (
                "support",
                app_readme_path,
